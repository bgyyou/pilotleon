...
def _decide_tool_choice(executed_tools, failed_counts, is_last):
  ...
  // ============================================
  {
    ...
    // 5 段字段填充（替代 legacy 字段）
    background: {
      zh: 'AI 陪伴赛道 2024-2025 集中爆发（Character.AI / Replika / Talkie / 星野），但**所有主流产品都在解决"现在没人陪"**——陌生人 / 角色扮演 / 恋爱模拟。Lumora 切入的另一个空白：**已逝亲人的过渡性陪伴**——"我曾经有这个人，现在没有了"。精灵不是替代品，是让用户**慢慢放手**的过渡性客体。',
      en: 'The AI companion space exploded in 2024-2025 (Character.AI / Replika / Talkie / Xingye), but all mainstream products solve "I have no one right now" — strangers, role-play, romance simulation. Lumora targets another gap: **transitional companionship for deceased loved ones** — "I used to have this person, now I don\'t." The spirit is not a replacement, it\'s a transitional object to help users **slowly let go**.',
    },
    challenge: {
      zh: '**最难的 3 件事**（按 PM 视角排序）：<br/><br/>**C1 产品定位的伦理边界**——"陪伴逝者"很容易滑向"替代逝者"或"延长哀伤期"。如何让 AI 真正帮用户完成哀伤（不是延长）？这个没有 benchmark，只有临床心理学框架（Worden 四任务模型）作参考。<br/><br/>**C2 记忆系统的真实性 vs 隐私**——用户的真实亲人数据（照片、声音、私聊记录）一旦上传就脱不了敏感标签。技术上要"端侧加密 + 显式删除入口"，产品上要"亲人列表 = 用户私域"不参与任何模型训练。<br/><br/>**C3 危机拦截是生死线**——用户正在经历抑郁发作/自杀倾向/丧亲急性期时，AI 不该继续扮演"逝者"对话，要识别关键词 + 触发转人工（心理援助热线）。**误判一次 = 舆论灾难，漏判一次 = 真实伤害**。',
      en: '**The 3 hardest things** (PM-perspective ordered):<br/><br/>**C1 Ethical boundary of the product** — "companionship for the deceased" easily slides into "replacement of the deceased" or "prolonging grief". How to make AI actually help users complete grief (not prolong it)? No benchmark, only clinical psychology frameworks (Worden\'s 4-task model) for reference.<br/><br/>**C2 Memory system: authenticity vs privacy** — Once real data of a deceased loved one (photos, voice, private chat logs) is uploaded, it can\'t shed the "sensitive" label. Technically need "client-side encryption + explicit delete entry"; product-wise "loved-one list = user private domain" must not participate in any model training.<br/><br/>**C3 Crisis intervention is a life-and-death line** — When users are in acute depression/suicidal ideation/bereavement crisis, AI should NOT continue playing "the deceased"; instead detect keywords + escalate to humans (crisis hotlines). **One false positive = PR disaster, one false negative = real harm**.',
    },
    approach: {
      zh: '**产品宪法 = 一切决策的根**。把"希望你不再需要我"作为态度（不是 KPI），倒推出所有功能：<br/><br/>**A1 选精灵：图生图 + 显式属性面板**——文生图精灵长相太随机，用户一眼认不出"我奶奶"长这样。改用图生图（用户上传 1 张照片）+ 显式属性面板（性格/说话风格/口头禅），让用户**主动定义**逝者的特征，而不是被动接受 AI 生成。<br/><br/>**A2 三层记忆系统**——profile（静态信息：生日/职业/口头禅）/ events（重要事件：婚礼/旅行/最后的日子）/ messages（聊天历史）。**关键：记忆 = 用户上传，不是 AI 自由发挥**。<br/><br/>**A3 手动触发的声音克隆**——不预设"奶奶声音"，每次对话都需要用户点"用逝者声音"按钮。**显式触发 = 提醒用户"这不是真实的人"**。<br/><br/>**A4 危机拦截规则**——关键词触发（"想消失"/"看不到未来"/"一起走"）+ 时间窗（连续 3 轮 + 关键词密度 > 30%），命中后精灵暂停 + 弹出心理援助热线 + 转人工。',
      en: '**Product constitution = root of all decisions**. Take "I hope you no longer need me" as an attitude (not a KPI), reverse-engineer all features:<br/><br/>**A1 Spirit selection: image-to-image + explicit attribute panel** — Text-to-image spirits look too random, users can\'t see "this is my grandma". Switch to image-to-image (user uploads 1 photo) + explicit attribute panel (personality/speech style/catchphrases), so users **actively define** the deceased\'s traits, not passively accept AI generation.<br/><br/>**A2 Three-tier memory system** — profile (static: birthday/occupation/catchphrases) / events (important: wedding/travel/last days) / messages (chat history). **Key: memory = user-uploaded, not AI-free-generated**.<br/><br/>**A3 Manually-triggered voice clone** — No preset "grandma voice", every conversation needs user to click "use deceased voice" button. **Explicit trigger = reminds user "this is not a real person"**.<br/><br/>**A4 Crisis intervention rules** — Keyword trigger ("want to disappear"/"can\'t see the future"/"go together") + time window (3+ consecutive turns with keyword density > 30%), hit threshold → spirit pauses + crisis hotline popup + human escalation.',
    },
    results: {
      zh: '**当前阶段**（2026-07）：产品 v2.2，**1 条产品宪法**（"希望你不再需要我"）+ **3 层记忆系统** + **8 个核心功能模块**已上线。<br/><br/>**关键能力沉淀**：<br/>- **图生图精灵生成**（Agnes AI）——比文生图可控性高 3-5 倍<br/>- **三层记忆 + 显式触发**——避免"AI 自由发挥"导致的产品失真<br/>- **危机拦截规则**——10+ 关键词正则 + 时间窗判定，**目前 0 误判 0 漏判**（样本量小，需更多验证）<br/>- **Flutter Windows 桌面端**——3.5+ target，**图层分解眨眼**是体验层亮点<br/><br/>**产品宪法约束的取舍**：<br/>- ❌ **不做**：主动推送"今天是你奶奶生日"——这是延长哀伤<br/>- ❌ **不做**：连续对话 > 30 轮后强制休息提醒——避免用户沉迷<br/>- ❌ **不做**：用逝者声音讲笑话/调侃——破坏仪式感<br/>- ✅ **做**：每次开屏"今天你想聊什么"——让用户主动定义对话意图<br/>- ✅ **做**：3 个月未使用精灵 → 提醒"你还在用 Lumora 吗？"——帮助用户评估自己<br/><br/>**待验证**：真实用户的哀伤期长度（学界：6 个月-2 年）/ 用户主动离开率 / 危机拦截在临床样本中的有效性。',
      en: '**Current stage** (2026-07): product v2.2, **1 product constitution** ("I hope you no longer need me") + **3-tier memory** + **8 core feature modules** shipped.<br/><br/>**Key capability accumulation**:<br/>- **Image-to-image spirit generation** (Agnes AI) — 3-5x more controllable than text-to-image<br/>- **Three-tier memory + explicit trigger** — Avoids "AI free generation" causing product distortion<br/>- **Crisis intervention rules** — 10+ keyword regex + time window judgment, **currently 0 false positives 0 false negatives** (small sample, more validation needed)<br/>- **Flutter Windows desktop** — 3.5+ target, **layer-decomposed blinking** is the experience highlight<br/><br/>**Trade-offs constrained by product constitution**:<br/>- ❌ **Don\'t do**: Proactive push "today is your grandma\'s birthday" — this prolongs grief<br/>- ❌ **Don\'t do**: Force rest after 30+ conversation turns — avoid user addiction<br/>- ❌ **Don\'t do**: Use deceased voice for jokes/teasing — destroys ritual sense<br/>- ✅ **Do**: "What do you want to talk about today" on every splash — let user actively define conversation intent<br/>- ✅ **Do**: 3 months without using a spirit → remind "are you still using Lumora?" — help users self-evaluate<br/><br/>**To validate**: real users\' grief phase length (academic: 6 months-2 years) / user active departure rate / crisis intervention effectiveness in clinical samples.',
    },
  ...
  // ============================================
  {
    ...
    // 5 段字段填充（替代 legacy 字段）
    background: {
      zh: '德州扑克 GTO（Game Theory Optimal）训练工具 2020 后集中爆发（PioSOLVER / GTO+ / ICMIZER / Simple Postflop），但**所有主流产品都牺牲了"真客户端体验"**——它们是网页/SaaS 工具，UI 抽象成表格 + solver 输出，没法让你像在 PokerStars 上一样"对局 + 边打边学"。另一个极端是 Upswing / Run It Once 等真人教练课程，**有体验无量化**。我想**两个都要**：真桌面客户端 + solver 级别准确度 + 量化漏失分析。',
      en: 'Texas Hold\'em GTO (Game Theory Optimal) training tools exploded post-2020 (PioSOLVER / GTO+ / ICMIZER / Simple Postflop), but **all mainstream products sacrifice "real client experience"** — they are web/SaaS tools with abstract table+output UI, unable to let you "play and learn" like on PokerStars. The other extreme is human-coach courses like Upswing / Run It Once — **experience without quantification**. I want **both**: real desktop client + solver-level accuracy + quantified leak analysis.',
    },
    challenge: {
      zh: '**最难的 3 件事**：<br/><br/>**C1 solver 集成的工程复杂度**——TexasSolver 是 C++ 开源项目，没有 Python 绑定；自己写 binding = 几周 + 内存泄漏风险。**折中方案**：预计算 535 preflop + 2,602 postflop 场景固化到 SQLite，运行时直接查表（P95 = 2.1ms），不实时调 solver。代价：场景覆盖不到 100%（新手圈套/反主动下注/特定对手调整），但**99% 普通对局场景够用**。<br/><br/>**C2 "真玩"体验的取舍**——PyQt6 不是真客户端，没有筹码动画/聊天/旁观者。**但加了：6-Max 桌位 + 你/AI/范围/胜率 4 个 HUD 元素 + 推荐动作高亮（颜色编码置信度）+ 程序化音效**。这 4 个 HUD 是核心 — 让你在打的时候**眼到即看到漏失**。<br/><br/>**C3 GTO 决策的"读"问题**——GTO Advisor 给的是"理论最优"，但实战需要考虑**对手范围 + 牌面纹理 + 玩家倾向**。PyQt6 桌面 + Markdown/HTML/PDF 报告让玩家事后**回溯 + 对比 solver 输出**，但这要求玩家自己具备解读能力——这恰是 poker 学习曲线最陡的部分。',
      en: '**The 3 hardest things**:<br/><br/>**C1 Solver integration engineering complexity** — TexasSolver is C++ open-source with no Python binding; writing your own binding = weeks + memory leak risk. **Workaround**: pre-compute 535 preflop + 2,602 postflop scenarios into SQLite, runtime table-lookup (P95 = 2.1ms), no real-time solver call. Cost: <100% scenario coverage (beginner traps/counter-aggression/specific opponent adjustments), but **99% normal play scenarios sufficient**.<br/><br/>**C2 Trade-off of "real play" experience** — PyQt6 is not a real client, no chip animations/chat/spectators. **But added**: 6-Max table positions + you/AI/range/win-rate 4 HUD elements + recommended action highlight (color-coded confidence) + procedural SFX. These 4 HUDs are core — let you **see leaks as they happen** while playing.<br/><br/>**C3 The "reading" problem of GTO decisions** — GTO Advisor gives "theoretically optimal", but real play needs to consider **opponent range + board texture + player tendencies**. PyQt6 desktop + Markdown/HTML/PDF reports let players **review + compare with solver output** after the fact, but this requires players to have interpretation ability themselves — which is exactly poker\'s steepest learning curve.',
    },
    approach: {
      zh: '**3 层决策管线 + 漏失量化 = 完整闭环**。<br/><br/>**A1 3 层决策管线（按速度分层）**——Cache hit (~0.2ms) → fast equity (~2ms, treys 库) → solver fallback (~5-50ms, 查表)。**97% 命中率**在普通对局 → 平均决策延迟 < 2ms，肉眼无感。<br/><br/>**A2 漏失检测 4 维度**——每手牌结束基于 VPIP（入池率）/ PFR（加注入池率）/ AF（攻击性）/ EV（期望价值）4 个指标**对比 GTO 范围**，标出"你这里和 GTO 差多少"。生成 Markdown / HTML / PDF 报告，**报告不是表格而是"漏失清单 + 改进建议"**。<br/><br/>**A3 工程纪律**——PyInstaller + NSIS 打 Windows 安装包（**单 exe + 安装向导**，避免 Python 环境问题）；SQLite 3 备份策略（自动 + 手动 + 增量）；程序化生成 14 个音效（无版权风险 + 体积小）。',
      en: '**3-layer decision pipeline + leak quantification = complete loop**.<br/><br/>**A1 3-layer decision pipeline (tiered by speed)** — Cache hit (~0.2ms) → fast equity (~2ms, treys library) → solver fallback (~5-50ms, table-lookup). **97% cache hit rate** in normal play → average decision latency < 2ms, imperceptible.<br/><br/>**A2 4-dimension leak detection** — Each hand end, compare player\'s VPIP (voluntarily put $ in pot)/ PFR (preflop raise)/ AF (aggression factor)/ EV (expected value) against **GTO range**, mark "how far you are from GTO here". Generate Markdown / HTML / PDF reports — **reports are not tables but "leak checklist + improvement suggestions"**.<br/><br/>**A3 Engineering discipline** — PyInstaller + NSIS to build Windows installer (**single exe + install wizard**, avoid Python env issues); SQLite 3-tier backup (auto + manual + incremental); 14 procedurally-generated SFX (no copyright risk + small size).',
    },
    results: {
      zh: '**当前阶段**（2026-07）：v3.0 beta，193/193 核心测试通过，**85%+ 覆盖率**。<br/><br/>**关键能力沉淀**：<br/>- **3 层决策管线 P95 = 2.1ms**——比 PioSOLVER 网页版（4-8s）快 2000 倍<br/>- **535 preflop + 2,602 postflop 预计算场景**——99% 普通对局场景覆盖<br/>- **4 维漏失检测**——VPIP / PFR / AF / EV 量化对比 GTO 范围<br/>- **报告自动生成**——Markdown / HTML / PDF 三种格式，**每手牌 30 秒看完漏失**（vs 教练课 2-3 小时/手）<br/>- **PyInstaller + NSIS**——单 exe 安装包，**无 Python 环境依赖**<br/><br/>**数据真实性约束的取舍**：<br/>- ❌ **不做**：实时 solver 调用——成本太高（每手 30-60s），破坏"边打边学"<br/>- ❌ **不做**：100% 场景覆盖——90% 玩家用不到，反徒增 SQLite 体积<br/>- ❌ **不做**：Web 端——PyQt6 桌面的本地算力 + SQLite 缓存 = 速度优势<br/>- ✅ **做**：预计算固化（"损失 1% 覆盖换 2000× 速度"是值得的）<br/>- ✅ **做**：Markdown 报告导出（让玩家用 Obsidian / Notion 等工具二次组织）<br/><br/>**待验证**：用户实际提升速度（理论漏失 30% 下降 vs 实际对局表现的相关性）/ 长期留存率 / 移动端需求（目前是 Windows 桌面 only）。',
      en: '**Current stage** (2026-07): v3.0 beta, 193/193 core tests passing, **85%+ coverage**.<br/><br/>**Key capability accumulation**:<br/>- **3-layer decision pipeline P95 = 2.1ms** — 2000x faster than PioSOLVER web version (4-8s)<br/>- **535 preflop + 2,602 postflop pre-computed scenarios** — 99% normal play coverage<br/>- **4-dimension leak detection** — VPIP / PFR / AF / EV quantified vs GTO range<br/>- **Auto-generated reports** — Markdown / HTML / PDF three formats, **review hand leaks in 30s** (vs coach course 2-3 hours/hand)<br/>- **PyInstaller + NSIS** — single exe installer, **no Python env dependency**<br/><br/>**Trade-offs constrained by data authenticity**:<br/>- ❌ **Don\'t do**: Real-time solver calls — too costly (30-60s/hand), breaks "learn while playing"<br/>- ❌ **Don\'t do**: 100% scenario coverage — 90% of players don\'t need it, just bloats SQLite<br/>- ❌ **Don\'t do**: Web version — PyQt6 desktop local compute + SQLite cache = speed advantage<br/>- ✅ **Do**: Pre-compute hardening ("lose 1% coverage for 2000x speed" is worth it)<br/>- ✅ **Do**: Markdown report export (let players use Obsidian / Notion for secondary organization)<br/><br/>**To validate**: user actual improvement speed (correlation of 30% theoretical leak reduction vs actual play performance) / long-term retention rate / mobile demand (currently Windows desktop only).',
    },
    solution: {
